...
<div class="pageHeader">
     <form id="pagerForm" onsubmit="return navTabSearch(this);" class="pageForm required-validate" action="answer.do" method="post">
		    ...
		    <table class="searchContent">
	           <tr>
			        ...
			        <td colspan="2">
			           ...
			           <select id="qid_index" name="answer.qid">
			               <option value="">全部</option>
			               <#if questionList??>
     ...
     <script language="javascript">
	     ...
	     function showQuestion(tid){
	         $("#qid_index option").remove();
	         $("#qid_index").append("<option value=''>全部</option>");
	         if(!tid) return;

	         $.getJSON("answer!getQuestion.do?tid="+tid,function(data) {
			     $.each(data.questionList,function(i,item){
			         $("#qid_index").append("<option value='"+item.id+"'>"+item.title+"</option>");
                 });      
			 });
	     }
	</script>
...
<div class="panelBar">
	...
	<table class="table" layoutH="140">
		<thead>
			<tr>
				<th style="text-align:center;">调查项目</th>
				<th style="text-align:center;">问题</th>
				<th style="text-align:center;">答案</th>
				<th style="text-align:center;">详细答案</th>
		     ...
		     <#if multiData.data??>
		       <#list multiData.data as rs>
					<tr target="sid" rel="${rs.id}">
						<td>${rs.topicTitle}</td>
						<td>${rs.questionTitle}</td>
						<td>${rs.answer}</td>
						<td>${rs.answerDetail}</td>
						<td>${rs.userName}</td>
